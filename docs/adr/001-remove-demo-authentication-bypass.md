...
> 후속 결정: 이 문서는 인증 우회를 제거할 당시의 역할 구조를 기록합니다. 이후 인증 우회 제거로 별도 기능이 없어진 `SUPER`를 `ADMIN`으로 통합했습니다. 현재 역할과 접근 제어 구조는 [ADR-006](006-centralize-manager-authorization.md)을 따릅니다.
...
이 결정 당시 `SUPER`는 관리자 기능 범위를 구분하기 위해 사용하던 역할이었습니다. 이 결정의 대상은 역할 자체가 아니라 해당 역할과 특정 계정에 연결되어 있던 인증 우회 동작이었습니다.
...
- 당시 `ADMIN`과 `SUPER` 모두 실제 발급된 OTP로 인증합니다.
...
- 이 결정 당시에는 `SUPER` 역할을 유지하되 다른 관리자와 같은 OTP 발급·검증 절차를 적용했습니다.
...
- 인증 우회 제거 시점에는 `SUPER`가 권한의 의미로만 사용되어 역할과 인증 책임이 분리됐습니다.
...
- 역할 통합 전 `SUPER` 계정도 동일한 2차 인증 절차를 거쳤습니다.
